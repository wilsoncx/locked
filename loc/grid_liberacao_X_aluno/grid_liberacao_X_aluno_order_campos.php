<?php
   ...
   session_start();
    $Ord_Cmp = new grid_liberacao_X_aluno_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class grid_liberacao_X_aluno_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $_POST, $_GET;
   ...
   $tab_def_campos['liberacao'] = "liberacao";
   $tab_converte["liberacao"]   = "liberacao";
   $tab_ger_campos['aluno'] = "on";
   $tab_def_campos['aluno'] = "aluno";
   $tab_converte["aluno"]   = "aluno";
   $tab_ger_campos['idliberacao_x_aluno'] = "on";
   $tab_def_campos['idliberacao_x_aluno'] = "idliberacao_X_aluno";
   $tab_converte["idliberacao_X_aluno"]   = "idliberacao_x_aluno";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       $ordem = (substr($campo_sort, 0, 1) == "+") ? "asc" : "desc";
       $campo = substr($campo_sort, 1);
       $_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['ordem_select'][$campo] = $ordem;
   }
?>
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (isset($_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
...
?>
       <OPTION value="<?php echo NM_encode_input($tab_def_campos[$NM_cada_field]); ?>" <?php echo $tmp_sel; ?>><?php echo $_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['labels'][$NM_cada_field]; ?></OPTION>
<?php
       }
   }
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
...
?>
             <OPTION value="<?php echo NM_encode_input($class . $NM_cada_field); ?>" selected><?php echo $class . $_SESSION['sc_session'][$sc_init]['grid_liberacao_X_aluno']['labels'][$tab_converte[$NM_cada_field]]; ?></OPTION>
<?php
       }
   }
?>
