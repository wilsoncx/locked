...
$old_error_handler = set_error_handler("form_liberacao_scriptcase_error_handler");

function form_liberacao_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_liberacao']['contr_erro']) && $_SESSION['scriptcase']['form_liberacao']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_liberacao->contr_form_liberacao->Apl_com_erro = true;
            if ($inicial_form_liberacao->contr_form_liberacao->NM_ajax_flag)
            {
                $inicial_form_liberacao->contr_form_liberacao->NM_ajax_info['errList']['geral_form_liberacao'][] = $msg;
                return;
            }
            form_liberacao_scriptcase_error_display($msg, $err_no);
        }
...
function form_liberacao_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_liberacao'])  && '' != $_SESSION['scriptcase']['error_icon']['form_liberacao'])  ? $_SESSION['scriptcase']['error_icon']['form_liberacao']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_liberacao']) && '' != $_SESSION['scriptcase']['error_close']['form_liberacao']) ? $_SESSION['scriptcase']['error_close']['form_liberacao'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_liberacao']) && '' != $_SESSION['scriptcase']['error_span_title']['form_liberacao']) ? $_SESSION['scriptcase']['error_span_title']['form_liberacao'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_liberacao']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_liberacao']) ? $_SESSION['scriptcase']['error_icon_title']['form_liberacao'] : '';
?>
