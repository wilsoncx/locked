<form name="F2" method=post 
               action="form_aluno_view_list_inline.php" 
               target="_self"> 
<input type=hidden name="idaluno_" value="<?php echo NM_encode_input($this->nmgp_dados_form['idaluno_']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_aluno_view_list_inline']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
</form> 
...
function scInlineFormSend()
{
  ...
  {
    parent.scInlineFormReceive(oResp, <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_aluno_view_list_inline']['inline_form_seq'] ?>);
    return true;
  }
  return false;
}
...
function NM_critica (form, campo, label, mask, tipo, intr, dec, min, max)
{
 ...
 NM_tab_crit[1] = new NM_critica('F1', 'nome_', 'Nome', 'naomask', 'lista', 'cxab', 80, 'TUDO');
 NM_tab_crit[2] = new NM_critica('NM_final');
 NM_tab_crit_1 = new NM_critica(2);
 NM_tab_crit_1[1] = new NM_critica('F1', 'nome_', 'Nome', 'naomask', 'lista', 'cxab', 80, 'TUDO');
