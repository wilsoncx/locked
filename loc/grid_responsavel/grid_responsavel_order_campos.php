<?php
   ...
   session_start();
    $Ord_Cmp = new grid_responsavel_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class grid_responsavel_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $_POST, $_GET;
   ...
   $tab_def_campos['idresponsavel'] = "idresponsavel";
   $tab_converte["idresponsavel"]   = "idresponsavel";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_responsavel']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_responsavel']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_responsavel']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_responsavel']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_responsavel']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_responsavel']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_responsavel']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_responsavel']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       $ordem = (substr($campo_sort, 0, 1) == "+") ? "asc" : "desc";
       $campo = substr($campo_sort, 1);
       $_SESSION['sc_session'][$sc_init]['grid_responsavel']['ordem_select'][$campo] = $ordem;
   }
?>
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (isset($_SESSION['sc_session'][$sc_init]['grid_responsavel']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
...
?>
       <OPTION value="<?php echo NM_encode_input($tab_def_campos[$NM_cada_field]); ?>" <?php echo $tmp_sel; ?>><?php echo $_SESSION['sc_session'][$sc_init]['grid_responsavel']['labels'][$NM_cada_field]; ?></OPTION>
<?php
       }
   }
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_responsavel']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
...
?>
             <OPTION value="<?php echo NM_encode_input($class . $NM_cada_field); ?>" selected><?php echo $class . $_SESSION['sc_session'][$sc_init]['grid_responsavel']['labels'][$tab_converte[$NM_cada_field]]; ?></OPTION>
<?php
       }
   }
?>
