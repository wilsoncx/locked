<?php
   ...
   session_start();
    $Ord_Cmp = new grid_horario_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class grid_horario_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $_POST, $_GET;
   ...
   $tab_def_campos['idhorario'] = "idhorario";
   $tab_converte["idhorario"]   = "idhorario";
   $tab_ger_campos['descricao'] = "on";
   $tab_def_campos['descricao'] = "descricao";
   $tab_converte["descricao"]   = "descricao";
   $tab_ger_campos['dia'] = "on";
   $tab_def_campos['dia'] = "dia";
   $tab_converte["dia"]   = "dia";
   $tab_ger_campos['h_entrada'] = "on";
   $tab_def_campos['h_entrada'] = "h_entrada";
   $tab_converte["h_entrada"]   = "h_entrada";
   $tab_ger_campos['h_saida'] = "on";
   $tab_def_campos['h_saida'] = "h_saida";
   $tab_converte["h_saida"]   = "h_saida";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_horario']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_horario']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_horario']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_horario']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_horario']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_horario']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_horario']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_horario']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       $ordem = (substr($campo_sort, 0, 1) == "+") ? "asc" : "desc";
       $campo = substr($campo_sort, 1);
       $_SESSION['sc_session'][$sc_init]['grid_horario']['ordem_select'][$campo] = $ordem;
   }
?>
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (isset($_SESSION['sc_session'][$sc_init]['grid_horario']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
...
?>
       <OPTION value="<?php echo NM_encode_input($tab_def_campos[$NM_cada_field]); ?>" <?php echo $tmp_sel; ?>><?php echo $_SESSION['sc_session'][$sc_init]['grid_horario']['labels'][$NM_cada_field]; ?></OPTION>
<?php
       }
   }
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_horario']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
...
?>
             <OPTION value="<?php echo NM_encode_input($class . $NM_cada_field); ?>" selected><?php echo $class . $_SESSION['sc_session'][$sc_init]['grid_horario']['labels'][$tab_converte[$NM_cada_field]]; ?></OPTION>
<?php
       }
   }
?>
