<?php
   ...
   session_start();
    ...
    $Sel_Cmp = new grid_liberacao_sel_cmp(); 
    $Sel_Cmp->Sel_cmp_init();
   
class grid_liberacao_sel_cmp
{
function Sel_cmp_init()
{
   ...
   {
       $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['field_order'] = $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['field_order_orig'];
       unset($_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['usr_cmp_sel']);
       $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['prim_cmp_sel'] = true;
   }
   if (!isset($_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['prim_cmp_sel']))
   {
       $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['prim_cmp_sel'] = true;
   }
   ...
   if (isset($_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (isset($_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['usr_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['usr_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['usr_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
...
function Sel_processa_out($opt_save=false)
{
   ...
   $arr_temp = array();
   if (!isset($_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['usr_cmp_sel']))
   {
       $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['usr_cmp_sel'] = array();
   }
   $arr_order = $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['field_order'];
   $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['field_order'] = array();
   $campos_sel = explode("@?@", $this->campos_sel);
   foreach ($campos_sel as $campo_order)
   {
       if (isset($tab_ger_campos[$campo_order]))
       {
           $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['field_order'][] = $campo_order;
       }
   }
   foreach ($arr_order as $campo_order)
   {
       if (!in_array($campo_order, $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['field_order']))
       {
           $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['field_order'][] = $campo_order;
       }
   }
   ...
   if ($arr_temp !== $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['usr_cmp_sel'])
   {
       $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['prim_cmp_sel'] = false;
   }
   $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['usr_cmp_sel'] = $arr_temp;
   if ($opt_save)
   {
       return;
   }
...
<?php
   ...
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           ...
           $prep_combo[strtolower($_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['labels'][$NM_cada_field])] = NM_encode_input($NM_cada_field) . '" ' . $tmp_sel . '>' . $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['labels'][$NM_cada_field];
       }
   }
...
<?php
   foreach ($_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['field_order'] as $NM_cada_field)
   {
       if ($tab_ger_campos[$NM_cada_field] == "on")
       {
?>
       <OPTION value="<?php echo NM_encode_input($NM_cada_field); ?>"><?php echo $_SESSION['sc_session'][$this->sc_init]['grid_liberacao']['labels'][$NM_cada_field]; ?></OPTION>
<?php
       }
   }
?>
