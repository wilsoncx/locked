...
$old_error_handler = set_error_handler("form_turma_scriptcase_error_handler");

function form_turma_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_turma']['contr_erro']) && $_SESSION['scriptcase']['form_turma']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_turma->contr_form_turma->Apl_com_erro = true;
            if ($inicial_form_turma->contr_form_turma->NM_ajax_flag)
            {
                $inicial_form_turma->contr_form_turma->NM_ajax_info['errList']['geral_form_turma'][] = $msg;
                return;
            }
            form_turma_scriptcase_error_display($msg, $err_no);
        }
...
function form_turma_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_turma'])  && '' != $_SESSION['scriptcase']['error_icon']['form_turma'])  ? $_SESSION['scriptcase']['error_icon']['form_turma']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_turma']) && '' != $_SESSION['scriptcase']['error_close']['form_turma']) ? $_SESSION['scriptcase']['error_close']['form_turma'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_turma']) && '' != $_SESSION['scriptcase']['error_span_title']['form_turma']) ? $_SESSION['scriptcase']['error_span_title']['form_turma'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_turma']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_turma']) ? $_SESSION['scriptcase']['error_icon_title']['form_turma'] : '';
?>
